...
├── create_tables.py         # One-off: create all DB tables
...
114 tests covering auth, technical indicators (cross-checked against pandas), the risk/decision engines, portfolio P&L, paper trading, the alert engine (including cooldown and crossover-detection edge cases), order validation, WebSocket auth, and broad API endpoint coverage. Tests run against an isolated in-memory SQLite database and mock all external APIs (Dhan, Gemini) — they never touch your real database or spend real API quota.
